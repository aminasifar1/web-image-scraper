...
from hf_utils import push_model_to_hub, create_model_card
...
def parse_option():
    ...
    parser.add_argument('--push-to-hub', action='store_true', help='whether to push model to Hugging Face Hub')
    parser.add_argument('--hub-repo-id', type=str, default='aminasifar1/TFG-model', help='Hugging Face repo ID (format: username/repo-name)')
    parser.add_argument('--hub-token', type=str, default=None, help='Hugging Face API token (or use HF_TOKEN env var)')
    parser.add_argument('--hub-create-model-card', action='store_true', help='whether to create a model card on Hub')
...
def main(config, log_writer, args=None):
    ...
    if dist.get_rank() == 0 and args is not None and args.push_to_hub:
        if args.hub_repo_id is None:
            logger.warning("--push-to-hub specified but --hub-repo-id not provided. Skipping Hub upload.")
        else:
            logger.info(f"Pushing model to Hugging Face Hub: {args.hub_repo_id}")
            push_success = push_model_to_hub(
            ...
            if push_success and args.hub_create_model_card:
                logger.info(f"Creating model card for {args.hub_repo_id}")
                create_model_card(
                    repo_id=args.hub_repo_id,
                    model_description=f"SPAI model trained with {config.DATA.DATASET}",
                    hf_token=args.hub_token,
                    logger=logger
                )
...
if __name__ == '__main__':
    ...
    main(config, log_writer, args=args)
